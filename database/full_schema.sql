-- =====================================================================
-- School Information System - Full Database Schema (consolidated)
-- MySQL / MariaDB
--
-- This single file recreates the COMPLETE, up-to-date database structure
-- (schema.sql + database/migrations/001_extend_enums.sql merged together).
-- Use it when moving the project to another PC.
--
-- Usage (from the project root, in a terminal):
--   mysql -u root -p < database/full_schema.sql
--
-- It will create the `school_information_system` database if it does not
-- exist, then (re)create all tables. WARNING: it DROPS existing tables,
-- so only run it on a fresh/empty database or when you intend to reset.
-- =====================================================================
...
-- Drop tables if they exist (child tables first to respect FKs).
...
-- Announcements table (targetAudience already extended per migration 001)
...
-- Events table (targetAudience already extended per migration 001)
CREATE TABLE events (
  id VARCHAR(191) PRIMARY KEY,
  title VARCHAR(191) NOT NULL,
...
-- Submissions table (type ENUM extended + targetAudience/targetId added per migration 001)
CREATE TABLE submissions (
  id VARCHAR(191) PRIMARY KEY,
  type ENUM('ANNOUNCEMENT', 'EVENT', 'REPORT', 'ATTENDANCE', 'PERFORMANCE', 'OTHER') NOT NULL,
  title VARCHAR(191) NOT NULL,
  content TEXT NOT NULL,
  targetAudience VARCHAR(32) NULL,
...
-- Approvals table (type ENUM extended per migration 001)
